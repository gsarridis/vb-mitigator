...
class MAVIASCTrainer(BaseTrainer):
    ...
    def _setup_models(self):
        ...
        if self.cfg.MITIGATOR.MAVIASC.BCC_PATH != "":
            bcc_net_dict = get_local_model_dict(self.cfg.MITIGATOR.MAVIASC.BCC_PATH)
    ...
    def _setup_optimizer(self):
        ...
        if self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.TYPE == "SGD":
            self.optimizer_projection = torch.optim.SGD(
                parameters_projection,
                lr=self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.LR,
                momentum=self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.MOMENTUM,
                weight_decay=self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.WEIGHT_DECAY,
            )
        elif self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.TYPE == "Adam":
            ...
            raise ValueError(
                f"Unsupported optimizer type: {self.cfg.MITIGATOR.MAVIASC.PROJNET.OPTIM.TYPE}"
            )
    ...
    def _train_iter(self, batch):
        ...
        with torch.no_grad():
            for _, bcc_net in self.bcc_nets.items():
                _, b_feats_in = bcc_net(inputs)
        b_feats = self.proj_net(b_feats_in)

        B = inputs.size(0)
        # k = int(0.9 * B)

        # mask = torch.zeros(B, device=self.device)
        # idx = torch.randperm(B, device=self.device)[:k]
        # mask[idx] = 1.0
        # mask = mask.view(B, 1)  # for broadcasting
        with torch.no_grad():
            b_feats_in = F.normalize(b_feats_in, dim=1)  # assume you have them
            sim = b_feats_in @ b_feats_in.T  # cosine similarity
            density = sim.sum(dim=1)  # how "crowded" each sample is

        k = int(0.95 * B)
        _, idx = torch.topk(density, k=k, largest=False)
        ...
        norm_loss = F.mse_loss(
            norm_main, norm_clip * self.cfg.MITIGATOR.MAVIASC.LOSS.LAMBDA
        )
        ce_loss = self.criterion(logits + mask * logits2, targets)

        loss = ce_loss + self.cfg.MITIGATOR.MAVIASC.LOSS.ALPHA * norm_loss
