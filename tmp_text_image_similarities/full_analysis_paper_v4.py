...
def _plot_isolated_factor(
    ...
    ax.set_xlabel(factor_label)
    ax.set_title(f"Isolated {factor_label} Effect on {metric_label}\n{name}")
    ax.grid(True, alpha=0.3, axis="x")
...
def _plot_isolated_training_data(
    ...
    ax.set_yticks(x)
    ax.set_yticklabels(datasets, fontsize=9)
    ax.set_xlabel(f"Mean {metric_label} Advantage (pp)")
    ax.set_title("Average Performance Advantage")
    ax.axvline(0, color="black", linewidth=0.5)
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()

    plt.suptitle(f"Training Data Comparison: {name}", fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def compute_feature_importance_rf(
    ...
    ax.set_title("By Factor Category")
    ax.grid(True, alpha=0.3, axis="y")
...
def analyze_interactions(
    ...
    ax.set_xlabel("Model Size (standardized)")
    ax.set_ylabel(f"{metric_label} (%)")
    ax.set_title("Architecture × Model Size")
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Dataset Size (standardized)")
    ax.set_ylabel(f"{metric_label} (%)")
    ax.set_title("Architecture × Dataset Size")
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Model Size (standardized)")
    ax.set_ylabel(f"{metric_label} (%)")
    ax.set_title("Model Size × Dataset Size")
    ax.legend()
    ax.grid(True, alpha=0.3)
